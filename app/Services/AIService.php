...
class AIService
{
    ...
    public static function callSync($messages)
    {
        $usedTools = [];
        $responseText = self::callClaudeSync($messages, $usedTools);
    ...
    private static function callClaudeSync($messages, &$usedTools = [])
    {
        $response = Http::withHeaders([
            'Content-Type' => 'application/json',
            'Authorization' => 'Bearer ' . env('TOKEN'),
        ...
        if ($stopReason === 'tool_use' && !empty($toolUseBlocks)) {
            $assistantContent = [];
            $toolResults = [];
            ...
            return self::callClaudeSync($messages, $usedTools);
        }
